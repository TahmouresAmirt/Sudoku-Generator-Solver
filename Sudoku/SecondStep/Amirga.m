...
CostFunction = @(sol,Sudoku)LastStage(sol,Sudoku);
...
[A,Sudoku] = CreateNewModel; 
if isempty(A)
    break;
end

for i=1:nPop
    pop(i).Position = CreateRandomSolution(A);
    pop(i).Cost = CostFunction(pop(i).Position,Sudoku);
end
...
for i = 1:MaxIt
            ...
            for k = 1:nc/2
               ...
               if UseRoulett
                ...
                popc(k,1).Cost = CostFunction(popc(k,1).Position,Sudoku);
                popc(k,2).Cost = CostFunction(popc(k,2).Position,Sudoku);
            end
          ...
          i1 = randi([1,nPop],1);
             ...
             popm(m).Cost = CostFunction(popm(m).Position,Sudoku);
           end
